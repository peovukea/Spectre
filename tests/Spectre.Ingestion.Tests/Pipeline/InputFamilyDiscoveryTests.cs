...
namespace Spectre.Ingestion.Tests.Pipeline;

public sealed class InputFamilyDiscoveryTests
{
    [Fact]
    public void DirectoryInput_GroupsAndOrdersFamiliesAndSegmentsGlobally()
    {
        ...
        var families = InputFamilyDiscovery.Resolve([temp.Path]);
    ...
    public void DirectSegmentInput_IsRejected()
    {
        ...
        Assert.Throws<InputValidationException>(
            () => InputFamilyDiscovery.Resolve([temp.File("family.bin.1")]));
    }
    ...
    public void DuplicatePhysicalFamilyAcrossInputs_IsRejected()
    {
        ...
        Assert.Throws<InputValidationException>(
            () => InputFamilyDiscovery.Resolve([temp.Path, basePath]));
    }
    ...
    public void MissingSegment_IsRejected()
    {
        ...
        Assert.Throws<InputValidationException>(
            () => InputFamilyDiscovery.Resolve([temp.Path]));
    }
}
